...
> **平台限制**：此 Skill 僅支援 macOS（依賴 DictionaryServices.framework）
...
| 依賴套件 | pyobjc-framework-DictionaryServices |
...
| 限制 | 說明 |
|------|------|
| 平台限制 | 僅限 macOS（依賴 DictionaryServices.framework） |
| 字典依賴 | 需在「字典」應用程式中啟用日文字典 |
| 查不到的情況 | 專有名詞、新詞、複合詞可能查不到 |
| 中日共用漢字 | 可能返回中文結果，需按上述策略處理 |

### 故障排除

**問題：查詢時顯示 "platform_unsupported"**
- 原因：非 macOS 系統
- 解決：此 Skill 僅支援 macOS

**問題：查詢時顯示 "not_found"**
- 原因：字典未啟用或詞彙不存在
- 解決：開啟「字典」App，確認已勾選日文字典

**問題：查詢結果為空**
- 原因：pyobjc 套件未正確安裝
- 解決：`uv pip install pyobjc-framework-DictionaryServices`
