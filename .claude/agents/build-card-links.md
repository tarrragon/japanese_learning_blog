...
## 與 card-structure-handler 和 build-card-content 代理人的協作
...
4. **build-card-content 代理人**：接收草稿卡片，補充完整內容
...
| 項目 | 草稿卡片 | 新建卡片 |
|------|---------|---------|
| **YAML frontmatter** | ✅ 已建立 | ❌ 需由 card-structure-handler 建立 |
| **檔案路徑** | ✅ 已存在 | ❌ 需由 card-structure-handler 建立 |
| **編號** | ✅ 已分配 | ❌ 需使用 get-next-number.py |
| **工作量** | 🟡 中等（補充內容） | 🔴 較大（結構+內容） |
| **CSV stage** | `draft` | `pending` |

build-card-content 代理人處理草稿卡片時：
- ✅ 跳過「取得編號」步驟（已有編號）
- ✅ 跳過「建立檔案」步驟（檔案已存在）
- ✅ 讀取現有 YAML frontmatter（保留 title、description、tags）
- ✅ 補充三語解釋、例句、詳細說明
- ✅ 更新 CSV 狀態為 `draft`
