...
```mermaid
flowchart TD
    A["要用哪個？<br/>ば還是たら？"] --> B{"什麼情況？"}

    B -->|"過去發生的事<br/>回到家時..."| C["必須用たら<br/>家に帰ったら、母がいた<br/>（回到家時，媽媽在）"]

    B -->|"日常會話<br/>邀請、請求"| D["優先用たら<br/>暇だったら、遊びに来てね<br/>（有空的話，來玩喔）"]

    B -->|"正式建議<br/>商務、演講"| E["優先用ば<br/>問題があれば、ご相談ください<br/>（如有問題，請商量）"]

    B -->|"一般真理<br/>格言諺語"| F["優先用ば<br/>努力すれば、道は開ける<br/>（努力的話，路就開）"]

    B -->|"時間順序<br/>做完A之後做B"| G["優先用たら<br/>仕事が終わったら、帰ります<br/>（工作結束後，回家）"]
