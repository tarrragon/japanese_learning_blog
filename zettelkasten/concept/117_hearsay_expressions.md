---
title: 伝聞表現（でんぶんひょうげん）
description: 傳聞表達的概念和語法體系
type: concept
jlpt: concept
stage: draft
tags:
...
flowchart TD
    A["要轉述他人的資訊"] --> B{"場合正式程度？"}
    B -->|"正式<br/>（書面/商務）"| C{"有明確來源？"}
    B -->|"中立<br/>（一般會話）"| D{"確定程度？"}
    B -->|"非正式<br/>（朋友間）"| E["〜って<br/>会議は中止になったって<br/>（聽說會議取消了）"]

    C -->|"有"| F["〜ということだ<br/>報告によると増加したということだ<br/>（根據報告增加了）"]
    C -->|"無"| G["〜とのことだ<br/>来週発売とのことです<br/>（聽說下週發售）"]

    D -->|"較確定"| H["〜そうだ<br/>明日は晴れるそうだ<br/>（聽說明天會放晴）"]
    D -->|"推測性"| I["〜らしい<br/>成功したらしい<br/>（好像成功了）"]

    style A fill:#607D8B,color:white
    style B fill:#795548,color:white
    style C fill:#9C27B0,color:white
    style D fill:#3F51B5,color:white
...
flowchart TD
    A["〜そうだ的兩種用法"] --> B{"資訊來源？"}
    B -->|"他人告知<br/>（傳聞）"| C["動詞原形 + そうだ<br/>雨が降るそうだ<br/>（聽說會下雨）"]
    B -->|"自己觀察<br/>（樣態）"| D["ます形去ます + そうだ<br/>雨が降りそうだ<br/>（看起來要下雨）"]
