---
title: ている（〜ている）
description: ている的多重語義功能：進行態、結果態、習慣態、經驗態
type: concept
subtype: aspect
jlpt: n4
stage: pending
...
```mermaid
flowchart TD
    A["ている<br/>（て形＋いる）"] --> B{"動詞類型 + 語境"}

    B -->|"活動動詞 + 當下時刻"| C["進行態<br/>雨が降っている<br/>（正在下雨）"]
    B -->|"瞬間動詞 + 狀態描述"| D["結果態<br/>結婚している<br/>（已婚狀態）"]
    B -->|"任意動詞 + 時間副詞"| E["習慣態<br/>毎日走っている<br/>（每天跑步）"]
    B -->|"知る・住む等 + 持續狀態"| F["經驗/狀態態<br/>知っている<br/>（知道）"]
...
flowchart LR
    ...
    style V4 fill:#9C27B0,color:white
