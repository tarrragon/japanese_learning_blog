...
```mermaid
flowchart TD
    subgraph 場景A["場景 A：公司內部"]
        A1["我"] --> A2["同事 = ウチ"]
        A1 --> A3["部長 = ソト<br/>（使用尊敬語）"]
    end

    subgraph 場景B["場景 B：對外部客戶"]
        B1["我"] --> B2["部長 = ウチ<br/>（使用謙讓語）"]
        B1 --> B3["客戶 = ソト<br/>（使用尊敬語）"]
    end
...
```mermaid
flowchart LR
    START["談論對象是誰？"] --> Q1{"是ウチ還是ソト？"}
    Q1 -->|"ウチ"| Q2{"說話對象是？"}
    Q1 -->|"ソト"| SONKEI["使用尊敬語"]
    Q2 -->|"對ソト說"| KENJOU["使用謙讓語"]
    Q2 -->|"對ウチ說"| CHECK["視情況：<br/>上司→尊敬語<br/>同輩→普通體"]

    style START fill:#795548,color:white
    style Q1 fill:#2196F3,color:white
    style Q2 fill:#2196F3,color:white
    style SONKEI fill:#4CAF50,color:white
    style KENJOU fill:#FF9800,color:white
