...
### 基礎文法
- [[grammar/particle_to.md]] - 引用助詞「と」的詳細用法（待建立）
- [[grammar/hearsay_souda.md]] - 傳聞表現「〜そうだ」（待建立）
- [[grammar/modal_souda.md]] - 様態表現「〜そうだ」（待建立）

### 相關動詞
- [[verb-u/iu.md]] - 言う（說）（待建立）
- [[verb-u/omou.md]] - 思う（想、認為）（待建立）
- [[verb-u/kiku.md]] - 聞く（聽）（待建立）
- [[verb-ru/tsutaeru.md]] - 伝える（傳達）（待建立）

### 延伸說明
- [[grammar/direct_indirect_speech.md]] - 直接引用與間接引用的對比（待建立）
- [[grammar/quotation_in_business.md]] - 商務場合的引用表現（待建立）
- [[honorific/quotation_polite_forms.md]] - 引用表現的敬語形式（待建立）

### 相關概念
- [[concept/information_source.md]] - 資訊來源與責任歸屬（待建立）
- [[concept/evidentiality.md]] - 證據性標記（待建立）

---

## 註解

[^grammar-pattern]: **文法模式** - 具有特定結構和功能的文法表達方式。詳見 [[concept/grammar_pattern.md]]（待建立）

[^particle-to]: **引用助詞「と」** - 用於標示引用內容的助詞，連接引用部分和引用動詞。詳見 [[particle/to_quotative.md]]（待建立）
...
[^tense]: **時態** - 動詞表示動作或狀態發生時間的文法範疇。日文主要有現在形和過去形。詳見 [[grammar/tense.md]]（待建立）

[^person]: **人稱** - 指示說話者、聽話者或第三者的文法範疇。日文的人稱主要透過代詞和語境表現。

[^hearsay]: **傳聞表現** - 表示「聽說」「據說」的文法表現，如「〜そうだ」「〜という」，用於傳達間接獲得的資訊。詳見 [[grammar/hearsay_expressions.md]]（待建立）
...
[^formal-context]: **正式場合** - 商務會議、公開演講、公文等需要使用正式語言的情境。詳見 [[context/formal.md]]（待建立）

[^polite-form]: **禮貌形式** - 丁寧語、尊敬語、謙讓語等表示禮貌的語言形式。詳見 [[concept/politeness.md]]（待建立）

[^business-context]: **商務場合** - 工作環境中的正式溝通情境，如會議、報告、商業信函等。詳見 [[context/business.md]]（待建立）
