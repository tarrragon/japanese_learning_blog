...
subtype: particle
jlpt: n5
tags: [grammar, particle, direction, comparison]
related_words: [kara, made, e]
...
**羅馬拼音**：e vs ni vs made

**品詞**：助詞[^joshi]（方向助詞[^direction-particle]）

## 日文解釋

「へ」「に」「まで」は、いずれも移動や方向を表す助詞[^joshi]ですが、使い方には重要な違いがあります。

**へ（e）**：大まかな方向[^general-direction]を示します。「〜の方へ」という意味で、到着点[^destination]よりも移動の方向性[^directionality]を強調します。

**に（ni）**：具体的な到着点[^destination]や目的地[^destination]を示します。「〜に到着する」という意味で、移動の終点[^endpoint]を明確に表します。

**まで（made）**：移動や動作の範囲の限界点[^limit-point]を示します。「〜まで行く」という意味で、「そこまで」という距離や範囲の終わりを強調します。出発点[^starting-point]を表す「から」と対をなす助詞です。

## 英文解釋

"へ (e)," "に (ni)," and "まで (made)" are all particles[^joshi] that express movement and direction[^direction-particle], but they have important distinctions in usage.

**へ (e)**: Indicates a general direction[^general-direction]. It emphasizes the directionality[^directionality] of movement rather than the specific destination[^destination], meaning "toward ~."

**に (ni)**: Indicates a specific arrival point[^destination] or destination[^destination]. It clearly expresses the endpoint[^endpoint] of movement, meaning "to ~ (and arriving there)."

**まで (made)**: Indicates the limit point[^limit-point] of movement or action range. It emphasizes the end of distance or scope, meaning "all the way to ~" or "until ~." It pairs with "から" (from) which indicates the starting point[^starting-point].

## 中文解釋

「へ」「に」「まで」都是表示移動和方向的助詞[^joshi]，但在使用方式上有重要區別。

**へ (e)**：表示大致的方向[^general-direction]。強調移動的方向性[^directionality]而非具體到達點[^destination]，意思是「朝向～」。

**に (ni)**：表示具體的到達點[^destination]或目的地[^destination]。明確表達移動的終點[^endpoint]，意思是「到～（並抵達）」。
