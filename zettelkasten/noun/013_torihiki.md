...
type: noun
jlpt: n3
tags: [business, transaction, commerce]
related_words: []
synonyms: []
created: 2025-10-31
...
「取引」は、商品[^shouhin]やサービス[^service]を売買[^baibai]することを指します。ビジネス[^business]の文脈でよく使用される用語で、契約[^keiyaku]に基づいて行われる商業活動[^shougyou]を表します。株式取引[^kabushiki]、為替取引[^kawase]、不動産取引[^fudousan]など、様々な種類の取引があります。

## 英文解釋

"Torihiki" refers to a transaction[^transaction] or trade, involving the buying and selling of goods[^shouhin] or services[^service]. It is commonly used in business[^business] contexts to describe commercial activities[^shougyou] conducted based on contracts[^keiyaku]. There are various types of transactions, including stock trading[^kabushiki], foreign exchange trading[^kawase], and real estate transactions[^fudousan].

## 中文解釋

「取引」指的是交易[^transaction]或買賣，涉及商品[^shouhin]或服務[^service]的購買和銷售。這是商業[^business]語境中常用的術語，用於描述基於契約[^keiyaku]進行的商業活動[^shougyou]。交易有多種類型，包括股票交易[^kabushiki]、外匯交易[^kawase]和不動產交易[^fudousan]等。

## 例句

1. この会社[^kaisha]は海外[^kaigai]との[^to]取引が盛んです[^sakan]。
   This company actively conducts overseas transactions.
   這家公司積極進行海外交易。

2. 株式取引[^kabushiki]には[^niwa]リスク[^risk]が伴います[^tomonau]。
   Stock trading involves risks.
   股票交易伴隨著風險。

3. 不動産取引[^fudousan]の契約書[^keiyakusho]を確認[^kakunin]してください。
   Please verify the real estate transaction contract.
   請確認不動產交易合約。

4. 公正[^kousei]な取引を心がけています[^kokorogakeru]。
